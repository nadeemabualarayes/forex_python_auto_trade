...
9. **News filter:** `news.py` pulls the Forex Factory weekly calendar (this week + next week), keeps the `NEWS_CURRENCIES` / `NEWS_IMPACTS` events (USD High by default) and blocks new entries from `NEWS_BLOCK_BEFORE_MIN` before to `NEWS_BLOCK_AFTER_MIN` after each release (15/15). The feed is refreshed every 4 hours (15 minutes after a failure) and the last good copy is kept in `logs/news_cache.json` so restarts do not re-fetch; a failed fetch keeps the cached copy, and once the cache is older than `NEWS_STALE_HOURS` the bot either trades blind (default) or stands aside (`NEWS_BLOCK_WHEN_UNAVAILABLE`). Open positions are not touched. The dashboard shows the next release and any active blackout; each blackout is announced once on Telegram. The backtester does not apply it (the feed has no history).
