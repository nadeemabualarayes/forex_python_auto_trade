...
# XAGUSD while the closed deals pay $1.0 / $5.0 per tick per lot, which sized a silver trade ten times over
# budget on 2026-09-08 (-$46.20 at a $5 stop). The terminal's own calculator (order_calc_profit) prices a
# move with the real contract size and currency conversion, so it is the primary source. The field is used
# only as a fallback for symbols where it has been checked against the terminal (verify_tick_value);
# otherwise no lot is placed. Ported from the scalp-test branch (commit 2b5a1b6).
