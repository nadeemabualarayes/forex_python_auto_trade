...
GOLD_TRUE = dict(GOLD, tick_value=1.0)      # contract 100 oz x 0.01: what the closed deals actually pay
SILVER_TRUE = dict(SILVER, tick_value=5.0)  # contract 5000 oz x 0.001
...
from execution import lot_for_loss, loss_per_lot, calculate_dynamic_lot, verify_tick_value, init_mt5  # noqa: E402
...
class TestVerifyTickValue:
    ...
    def test_init_mt5_verifies_every_selected_symbol(self, monkeypatch, caplog):
        ...
        monkeypatch.setattr(config, "MT5_PATH", "", raising=False)
        monkeypatch.setattr(config, "MT5_LOGIN", "", raising=False)
        with caplog.at_level("INFO", logger="bot"):
